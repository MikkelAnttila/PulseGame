...
public class HybridInverseKinematicsNode : MonoBehaviour
{
    ...
    public void ReprocessJoints()
    {
        ...
        foreach (HybridIKJoint n in nodes)
        {
            n.isStretchNode = n.jointTransform.CompareTag("HybridIKStretchNode");
    ...
    public void ReinitializeTransforms()
    {
        ...
        {
            node.jointTransform.localPosition = node.initialPosition;
            node.jointTransform.localRotation = node.initialRotation;
        }
    }
    ...
    public void ApplyJointLimits(HybridIKJoint parentNode, float jointLimitStrength = 1)
    {
        ...
        if (parentNode.isStretchNode) parentNode.jointTransform.localRotation = parentNode.initialRotation;
    ...
    public void FABRIK_Step(int numIterations = 2, bool useConstraints = false)
    {
        ...
        foreach (HybridIKJoint targetIKNode in ikTargetNodes)
        {
            ...
            for (int i = 0; i < nodes.Count; i++)
            {
                ...
                if (node.parent != null)
                {
                    if (HasPositionConstraint(targetIKNode))
                    {
                        targetIKNode.jointTransform.localPosition = targetLocalPosition;
                    }
        ...
        foreach (HybridIKJoint targetIKNode in ikTargetNodes)
        {
            ...
            for (int i = nodes.Count - 1; i > 0; i--)
            {
                ...
                if (node.parent != null)
                {
                    ...
                    if (useConstraints && HasPositionConstraint(targetIKNode))
                    {
                        targetIKNode.jointTransform.localPosition = targetLocalPosition;
                    }
                }
    ...
    public void CCD_Step(bool useConstraints = false)
    {
        if (!enableJointLimits) return;
    ...
    public void SwingAlignToParticle(HybridIKJoint parentNode, Vector3 originalPos, Vector3 targetPos, float jointLimitStrength = 1, bool IKMode = false, bool solvingForEndNode = false)
    {
        ...
        if (parentNode.isStretchNode)
            parentNode.jointTransform.localRotation = parentNode.initialRotation;
    ...
    public Vector3 ProjectEndTargetToConstraintsHull(Vector3 targetPoint)
    {
        ...
        if (IKHullVerts.Count > 3)
        {
            ...
            bool isInside;
            Vector3 projectedPoint = GeometryMathHelperUtils.ProjectPointOnConvexHull(IKHullVerts, IKHullTris, targetPoint, out isInside);
            ////return projectedPoint;
            if (isInside)
                return projectedPoint;
            return GeometryMathHelperUtils.FindClosestPointToConvexHull(IKHullVerts, IKHullTris, targetPoint);
        }
    ...
    void LateUpdate()
    {
        ...
        if (initializePoseFrom == InitializePoseFrom.INITIAL_POSE)
        {
            ResetToZeroPose();
        }
        ...
        // Apply first pass with along chain constraints
        FABRIK_Step(2, true);
        //DrawDebugSkeleton();
